...
import foxit.bean.CurrentState;
import foxit.bean.PDFContext;
import foxit.bean.PSIOperation;
import foxit.popupmenu.EditWindows;
import foxit.popupmenu.MenuItem;
import foxit.popupmenu.PopupMenu;
import foxit.task.TaskManager;
...
//		}
	...
	PopupMenu.OnItemOnClickListener manageAnnotListener = new PopupMenu.OnItemOnClickListener()
	{
	...
	public void initPsi()
	{
		...
		this.psiOperation.init(currentState.display_width * currentState.scale , currentState.display_height * currentState.scale);
	...
	private boolean onHighlightAndUnderlineTouchEvent(MotionEvent event, RectF rectF)
	{
		...
		case MotionEvent.ACTION_UP:
			...
			Annot.QuadpointsF[] quadpointsF = new Annot.QuadpointsF[annotRectFs.size()];
			for (int i = 0; i < annotRectFs.size(); i++)
			{
				RectF rect = annotRectFs.elementAt(i);
				//pdf coordinates
				quadpointsF[i] = new Annot.QuadpointsF();
